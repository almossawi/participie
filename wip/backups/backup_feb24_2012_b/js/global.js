...
function magnify_all(node, tween_duration, new_percent) {
	...
	if (parent = node.parent) { //i.e. for all nodes except the center one, which we don't care about for now
		var parent,
			...
			min_expand,
			max_expand,
			old_percent = node.dx / parent.dx;
			
		//first, find out how much new space we covered via the node's increase (DO DECREASE LATER)
		var percent_difference = Math.abs(new_percent - old_percent);
		console.log("percent difference: " + percent_difference);
				
		//then determine the new dx of that newly covered space
		var new_dx_of_that_percent_difference = parent.dx * percent_difference;
				
		//then split that dx per siblings and take tha off each's dx
		var dx_to_take_off_per_sibling = new_dx_of_that_percent_difference / (parent.children.length-1);
				
		console.log("node.dx: " + node.dx);
		console.log("parent.dx: " + parent.dx);
		console.log("percent_difference: " + percent_difference);
		console.log("new_percent: " + new_percent);
		console.log("old_percent: " + old_percent);
		console.log("new_dx_of_that_percent_difference: " + new_dx_of_that_percent_difference);
		console.log("(parent.children.length-1): " + (parent.children.length-1));
		console.log("dx_to_take_off_per_sibling: " + dx_to_take_off_per_sibling);
				
				
		//reposition this node and all of its siblings
		parent.children.forEach(function(sibling) {			
   			//for the node we just clicked
			if(sibling === node) {
				//we're increasing a constant value
  				var sibling_dx_temp = parent.dx*new_percent;
  				
  				console.log("=====================" + sibling.label);
				console.log("node's dx before resizing: " + sibling.dx);
				console.log("node's dx after resizing: " + sibling_dx_temp);
			...
			//for the node's homies
				...
				//finally, we have our sibling node's new dx that we'll be resizing to
				var sibling_dx_temp;
				if(old_percent > new_percent)
					sibling_dx_temp = sibling.dx + dx_to_take_off_per_sibling;
				else
					sibling_dx_temp = sibling.dx - dx_to_take_off_per_sibling;
				...
				console.log("=====================" + sibling.label);
				console.log("sibling's dx before resizing: " + sibling.dx);
				console.log("sibling's dx after resizing: " + sibling_dx_temp);
				console.log("this sibling as a % of siblings' total area is [" + sibling.label + "]: " + percent);
...
function update_ratios_and_redraw() {
	...
	for(var i=1;i<partition_nodes.length;i++) {
		...
		//update node's dx based on user input
		var percent = $("#ssn_percent").val();
		
		magnify_all(node, 1250, percent);
	}
}
