  ...
  (function() {
    ...
    var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(ga, s);
		...
		<div id="wall_of_pies_content">
			<div style="width:230px;position:absolute;left:70px;top:439px">
				<select id='show_filter'>
					<option value='all'>Latest</option>
					<optgroup label="Gender">
						<option value='M'>Females</option>
						<option value='F'>Males</option>
					</optgroup>
			...
			$result = mysql_query("select count(id) as n from cooked_pies_topics_to_discuss");
			
			if($result) {
				$row = mysql_fetch_assoc($result);
				echo "<h2 style='display:block;text-align:center;margin-left:auto;margin-right:auto'>" . $row['n'] . " delicious pies baked so far!</h2>";
			}
			...
			$rs_avg_pie_data = getAverageTalkingPointsPie("politics='RS'");
			$rs_avg_pie_count = $rs_avg_pie_data[0]; $rs_avg_pie = $rs_avg_pie_data[1];
			$rl_avg_pie_data = getAverageTalkingPointsPie("politics='RL'");
			$rl_avg_pie_count = $rl_avg_pie_data[0]; $rl_avg_pie = $rl_avg_pie_data[1];
			$nn_avg_pie_data = getAverageTalkingPointsPie("politics='NN'");
			$nn_avg_pie_count = $nn_avg_pie_data[0]; $nn_avg_pie = $nn_avg_pie_data[1];
			$dl_avg_pie_data = getAverageTalkingPointsPie("politics='DL'");
			$dl_avg_pie_count = $dl_avg_pie_data[0]; $dl_avg_pie = $dl_avg_pie_data[1];
			$ds_avg_pie_data = getAverageTalkingPointsPie("politics='DS'");
